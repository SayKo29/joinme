...
export default function EventDetailScreen({ route }) {
  ...
  const handleJoinEvent = () => {
    ...
    setLoading(true);
    mutate(
      { event: event, participant: userLogged.user },
      {
        onSuccess: () => {
    ...
    if (param === 1) {
      ...
      let eventToUpdate = {
        ...
        participants: event.participants.filter(
          (participant) => participant !== userLogged.user._id
        ),
      };
      ...
      {!loading && (
        <>
          <Headerback
            ...
            userHasJoinedEvent={userHasJoinedEvent}
          ...
          <View>
            ...
            >
              ...
              {userLogged.user._id !== event.user && !userHasJoinedEvent && (
                <TouchableOpacity
              ...
              {userHasJoinedEvent && (
                <TouchableOpacity style={styles.buttonJoin} disabled>
                  <Text style={styles.buttonText}>
                    Ya te has unido al evento
                  </Text>
                </TouchableOpacity>
              )}
              ...
              {eventCreator && (
                <Text style={[styles.space, styles.descriptionTitle]}>
                  Evento creado por:{" "}
                  <Text style={styles.text}>{eventCreator?.name}</Text>
                </Text>
              )}
